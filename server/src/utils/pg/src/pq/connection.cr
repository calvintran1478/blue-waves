...
module PQ
  ...
  class Connection
    ...
    def send_bind_message(params : Enumerable(Bytes) | Tuple())
      ...
      soc.write_byte('B'.ord.to_u8)
      soc.write_bytes((14 + (2 * nparams) + total_size), IO::ByteFormat::NetworkEndian)
      soc.write_byte(0_u8) # unnamed destination portal
      soc.write_byte(0_u8) # unnamed prepared statment

      soc.write_bytes(nparams.to_i16, IO::ByteFormat::NetworkEndian)
      nparams.times { soc.write_bytes(0_i16, IO::ByteFormat::NetworkEndian) }
      soc.write_bytes(nparams.to_i16, IO::ByteFormat::NetworkEndian)

      params.each do |param|
        soc.write_bytes(param.size, IO::ByteFormat::NetworkEndian)
        param.each { |byte| write_byte byte }
      end

      soc.write_bytes(1_i16, IO::ByteFormat::NetworkEndian)
      soc.write_bytes(1_i16, IO::ByteFormat::NetworkEndian)
    end
